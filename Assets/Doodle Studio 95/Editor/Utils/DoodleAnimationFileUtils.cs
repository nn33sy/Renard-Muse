	...
	internal static Object SaveAsSpritesheet(this DoodleAnimationFile file) {
		...
		try {
			...
			if (replacingTextureAsset || settings.textureType == TextureImporterType.Sprite) {
				...
				var spriteData = new SpriteMetaData[file.frames.Count];
				const int SPRITE_CENTER_ALIGNMENT = 9;
				...
				foreach (var kf in file.frames) {
					...
					if (rectIndex < settings.spritesheet.Length)
						smd = settings.spritesheet [rectIndex];
					
					smd.pivot = Vector2.one * 0.5f;
					smd.alignment = SPRITE_CENTER_ALIGNMENT;
					smd.border = file.spriteBorder;
					smd.name = "frame_" + rectIndex + "_len_" + kf.m_Length;
					if (rectIndex == 0) {
						smd.name += "_fps_" + (int)file.framesPerSecond;
						smd.name += "_playbackmode_" + (int)file.playbackMode;
					}
					smd.rect = rects [rectIndex];
					spriteData [rectIndex] = smd;
					rectIndex++;
				...
				settings.spritesheet = spriteData;
			}
			AssetDatabase.ImportAsset (outAssetPath, ImportAssetOptions.ForceSynchronousImport);
